/**
 * Alternative implementation for ISM2's first optimization.
 * The difference between this kernel and kernelTsm2 is that
 * values from C, the result matrix, are prefetched in a similar
 * manner to how A's and B's values are fetched.
 *
 * by Cody Rivera, 2019-2020
 */

#ifndef _KERNEL_ISM2_OPT1_CUH
#define _KERNEL_ISM2_OPT1_CUH

// #include <wb.h>
#include <hip/hip_runtime.h>

template <typename FloatType, int t1, int t2, int t3>
__global__ void kernelIsm2Opt1(const FloatType* A, const FloatType* B,
                ...
                // If there is another tc-row tile of A to consider
                    ...
                    for (int i = 0; i < t2; ++i) {
                        if (i < n && thread + (blockDim.x * gridDim.x) < m) {
                            nextC[i] =
                                C[thread + (blockDim.x * gridDim.x) + (i * m)];
                        }
                    }
                }
